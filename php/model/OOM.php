...
*/
	...
	class OOM{
		...
		* IT ONLY WORKS ON ALL, WHERE, FIND, FIND_BY AND LIKE
		*/
		public $private = [];
		public $attr = [];
		...
		*/
		...
		function all($params = [], $full_obj = true){
			...
			while ($row = $result__->fetch_assoc()) {
				if ( is_bool(array_search($row, $this->private)) ) {
		...
		*/
		function find($id, $full_obj = true){
			...
			$logger = new Logger();
			if (is_numeric($value)) {
				$result = $conn->query("SELECT * FROM ".$this->model_name."  id=".$id);
			}else{
		...
		*/
		...
		function where($value, $full_obj = true){
		    ...
		    $logger->log($this->private);
		    for ($i=0; $i < count($result); $i++) {
		    	$obj__ = "return new ".get_class($this)."();";
		    	$obj =  eval($obj__);

		    	for ($j=0; $j < count($result[$i]); $j++) {
		    		$logger->log(key($result[$i]));
		    		if ( is_bool(array_search(key($result[$i]), $this->private)) ) {
		    			$obj->attr[key($result[$i])] = $result[$i][key($result[$i])];
		    		}
		    		//echo key($result[$i])."<br>";
		    		next($result[$i]);
		...
		function last($or='', $where = '', $full_obj = true){
			...
			while ($current = current($z)) {
				if ( is_bool(array_search(key($z), $this->private)) ) {
